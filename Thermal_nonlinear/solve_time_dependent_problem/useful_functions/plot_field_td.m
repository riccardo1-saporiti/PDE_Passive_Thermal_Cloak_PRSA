function [fig] = plot_field_td(fig,field_data,plot_data,fonts_data)
...
pdeplot(field_vertices,field_elements(1:3,:),'xydata',y(field_indexes,tt),'xystyle','interp',...
        'zdata',y(field_indexes,tt)-2*abs(max(y(field_indexes,tt))) ,'zstyle','continuous',...
...
view(0,90)
h = colorbar;
set(h, 'ylim', [min( min(field_data.y))  max(max(field_data.y))]) %if want to set these limits manually use this command
