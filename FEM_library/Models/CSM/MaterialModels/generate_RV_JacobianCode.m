...
syms P_F00 P_F01 P_F02 P_F10 P_F11 P_F12 P_F20 P_F21 P_F22;

syms alpha beta bulk mu_q I_C pow23J pow2J vol_factor1 vol_factor2 logJ scalFact P_F

D = [u1; u2; u3];

F = [f1x f1y f1z; f2x f2y f2z; f3x f3y f3z];

P_F = [P_F00 P_F01 P_F02; P_F10 P_F11 P_F12; P_F20 P_F21 P_F22];
...
% Expanded Version
% DF =    - 2/3 * mu_q * pow23J * sum(sum(invFT.*GradU))*F ...
%         + 2/9 * mu_q *  pow23J * I_C * sum(sum(invFT.*GradU))*invFT ...
%         - 2/3 * mu_q * pow23J * sum(sum(F.*GradU))*invFT ...
%         +       mu_q * pow23J * GradU ...
%         + 1/3 * mu_q * pow23J * I_C * invFT * GradU.' * invFT ...
%         + 2 * pow23J * 2 * beta * 2 * pow23J * sum(sum((F - 1/3 * I_C * invFT).*GradU)) * (F - 1/3 * I_C * invFT) ...
%         + bulk * 0.5 * (2 * pow2J - J + 1) * sum(sum(invFT.*GradU))*invFT ...
%         + bulk * 0.5 * (J - pow2J - logJ) * invFT * GradU.' * invFT;

% Faster Version
DF =    - 2/3 * mu_q * pow23J * sum(sum(invFT.*GradU))*P_F ...
        - 2/3 * mu_q * pow23J * sum(sum(F.*GradU))*invFT ...
        +       mu_q * pow23J * GradU ...
        + 1/3 * mu_q * pow23J * I_C * invFT * GradU.' * invFT ...
        + 2 * pow23J * 2 * beta * 2 * pow23J * sum(sum(P_F.*GradU)) * P_F ...
        + vol_factor1 * sum(sum(invFT.*GradU))*invFT ...
        + vol_factor2 * invFT * GradU.' * invFT;
...
TableSubs = {'J', 'detF[q]';...
             ...
             'pow23detF[q]^2', 'pow23detF[q]*pow23detF[q]'; ...
             '/3', '/3.0';...
             '/2', '/2.0';...
             '/9', '/9.0';
             'P_F00', 'P_F[0][0]';...
