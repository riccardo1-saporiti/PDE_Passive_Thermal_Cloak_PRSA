...
void mexFunction(int nlhs,mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    double* dim_ptr = mxGetPr(prhs[0]);    /* MESH.dim */
    int dim     = (int)(dim_ptr[0]);       /* MESH.dim */
    int noe     = mxGetN(prhs[3]);         /* MESH.elements */
    double* nln_ptr = mxGetPr(prhs[4]);    /* FE_SPACE.numElemDof */
    int nln     = (int)(nln_ptr[0]);      
    int numRowsElements  = mxGetM(prhs[3]); /* MESH.elements */
    int nln2    = nln*nln;
    
    double* tmp_ptr1 = mxGetPr(prhs[2]);    /* dt */
