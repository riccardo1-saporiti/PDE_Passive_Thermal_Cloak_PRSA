...
for outiter = 1 : outer
    ...
    for initer = 1 : inner
        ...
        if verbosity(1) && mod((outiter-1)*inner+initer+1,verbosity(2)) == 0
            fprintf('\n  gmres iter %d: Rel_res = %2.4e',(outiter-1)*inner+initer+1,normr_act / n2minv_b);%resvec((outiter-1)*inner+initer+1)/n2b);   
        end
...
if verbosity(1)
      fprintf('\n  gmres iter %d: Rel_res = %2.4e \n',length(resvec),relres);%resvec((outiter-1)*inner+initer+1)/n2b);
end
