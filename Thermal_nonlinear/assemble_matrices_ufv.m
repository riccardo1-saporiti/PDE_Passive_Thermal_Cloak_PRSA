...
%digit pdeModeler in the command window to define the mesh and the other part; open it and open
%pdeModeler_layout_classic. Run pdeModeler. Boundary, Boundary Mode, Show
%Subdomain Labels
% The omega_obs is 1, the control
%is in 2. 
...
load(strcat("mesh_data",sslash,"mesh_obstacle_1_refinement_2_ufv"));
% 
%mesh_pet = to_save;
...
%mesh_obstacle_coarse_5: mesh refined in the region of the control:
%region2; two refinements in the same region
...
%%uncomment lines 157,158,278,279 to use it 
...
r  = 0.025;
source_strength = 1;
%rbf_p =@(x,y,x_0,y_0,sigma) exp( - ( (x-x_0).^2 + (y-y_0).^2 ) / sigma^2 );
id_source = @(x,y) ( ((x-x0).^2 + (y-y0).^2 - r) <=0 ) * source_strength;
%id_source = @(x,y) rbf_p(x,y,x0,y0,0.1) * source_strength; 
DATA.force = @(x,y,t,param)( id_source(x,y) );
% % % % % xs = linspace( -1 , 1 );
% % % % % ys = linspace( -1 , 1);
% % % % % [ xx , yy ] = meshgrid( xs , ys );
% % % % % surf( xs , ys , id_source( xx , yy ) )
% % % % % colormap( winter )
...
% % T_x_obs_test = sparse( size(T_x,1),size(T_x,2));
% % T_x_obs_test( observation_basis_index , observation_basis_index ) = T_x( observation_basis_index , observation_basis_index );
% % T_x_obs_test = T_x_obs_test(nodes_ocp_in,nodes_ocp_in);
% % T_x_obs      = T_x_obs_test;

% % T_y_obs_test = sparse( size(T_y,1),size(T_y,2));
% % T_y_obs_test( observation_basis_index , observation_basis_index ) = T_y( observation_basis_index , observation_basis_index );
% % T_y_obs_test = T_y_obs_test(nodes_ocp_in,nodes_ocp_in);
% % T_y_obs      = T_y_obs_test;
